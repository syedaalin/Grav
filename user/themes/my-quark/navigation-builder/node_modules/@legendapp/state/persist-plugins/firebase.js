...
const { symbolDelete, getPathType, clone } = state.internal;
...
class ObservablePersistFirebaseBase {
    ...
    async get(params) {
        ...
        if (waitForGet) {
            await state.whenReady(waitForGet);
        }
        const saveState = {
            pendingSaveResults: new Map(),
            pendingSaves: new Map(),
            numSavesPending: state.observable(0),
        };
    ...
    async set({ options, changes, obs }) {
        ...
        if (status$.numWaitingCanSave.peek() > 0) {
            ...
            await state.when(() => status$.numWaitingCanSave.get() < 1);
        }
        if (waitForSet) {
            await state.whenReady(waitForSet);
        }
        const saveState = this.saveStates.get(obs);
        // Don't start a save until all outstanding saves are finished
        await state.when(() => saveState.numSavesPending.get() === 0);
        ...
        for (let i = 0; i < changes.length; i++) {
            ...
            const value = state.constructObjectWithPath(path, pathTypes, clone(valueAtPath));
                    ...
                    for (let i = 0; i < saved.length; i++) {
                        ...
                        if (dateModified) {
                            ...
                            if (dateModifiedKeyOption) {
                                const deconstructed = state.deconstructObjectWithPath(path, pathTypes, value);
                                // Don't resurrect deleted items
                                if (deconstructed !== symbolDelete) {
                                    Object.assign(changesOut, state.constructObjectWithPath(path, pathTypes, {
    ...
    async _onceValue(path, pathTypes, pathFirebase, dateModifiedKey, dateModifiedKeyOption, queryByModified, onLoad, params, status$, snapshot) {
        ...
        if (outerValue && state.isObject(outerValue)) {
            ...
            value = state.constructObjectWithPath(path, pathTypes, value);
    ...
    async _onChange(path, pathTypes, pathFirebase, dateModifiedKey, dateModifiedKeyOption, params, localState, saveState, status$, snapshot) {
        ...
        if (numLoading > 0) {
            ...
            if (!startedLoading)
                return;
            // Wait for load
            await state.when(() => status$.numLoading.get() < 1);
        }
        const { onChange, state: state$1, options: { remote }, } = params;
        ...
        if (val) {
            ...
            const constructed = state.constructObjectWithPath(pathChild, pathTypes, value);
